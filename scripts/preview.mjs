...
for(const m of projects){
  ...
  if(m.kind==='profile'){
    await fs.cp(path.join(workspace,m.dir,'assets/live'),path.join(destination,'live'),{recursive:true});
  }
  const widgets=m.kind==='profile'?'<details open><summary>API 项目卡片 / 每 6 小时计划刷新</summary><p>真实 API 采集快照，非即时查询；更新时间见卡片。</p>'+['flowloud','mingxu','bilidigest','kehua'].map(id=>'<img class="live-card" src="'+m.dir+'/live/'+id+'-light.svg" data-light="'+m.dir+'/live/'+id+'-light.svg" data-dark="'+m.dir+'/live/'+id+'-dark.svg" alt="'+id+' live project metadata" width="400">').join(' ')+'</details>':'';
  ...
  sections+='<section id="p'+m.dir+'"><div class="section-head"><h2>'+esc(m.displayName)+'</h2><a href="'+esc(reviewUrl)+'" target="_blank" rel="noreferrer">'+(delivery?'查看草稿 PR':'Repository')+' ↗</a></div><img class="hero" src="'+m.dir+'/hero-light.svg" data-light="'+m.dir+'/hero-light.svg" data-dark="'+m.dir+'/hero-dark.svg" alt="'+esc(m.displayName+' — '+m.tagline.en)+'"><div class="caption"><p>'+esc(m.tagline.zh||m.tagline.en)+'</p><a href="'+m.dir+'/social-preview.png">Social preview · 1280×640 ↗</a></div>'+proof+widgets+'</section>';
}
